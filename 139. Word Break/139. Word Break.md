...
- 時間計算量： $n = len(s),\ m = len(wordDict)$ とすると、メモ化により `word_check(start)` は各 `start` について高々1回だけ計算されるので状態数は $O(n)$。各状態で `end` を最大 $O(n)$ 個試し、`wordDict` はこの時点では `list` なので `s[start:end] in wordDict` の探索は最大 $O(m)$。したがって上界は $O(n^2 \times m)$。
- 空間計算量：メモ化キャッシュが `start` ごとに高々1件で $O(n)$、再帰スタックも最大 $O(n)$ なので、全体で $O(n)$。
